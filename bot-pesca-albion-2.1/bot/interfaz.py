...
class BotWindow(QWidget):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("ARGENBOTS DE PESCA 2.1V")
        ...
        # Título
        self.title = QLabel("🎣 ARGENTBOTS DE PESCA v2.1", self)
        ...
        self.btn_pause.setStyleSheet(
            "background-color: #FFA500; color: white; font-size: 16px; padding: 10px; border-radius: 10px;"
        )
    ...
    def run_script(self):
        try:
            from main import iniciar_desde_interfaz, verificar_recursos

            # Primero mostrar la ruta y contenido de recursos para depurar
            verificar_recursos(log_callback=self.logSignal.emit)
            iniciar_desde_interfaz(
                log_callback=self.logSignal.emit, pause_event=self.pause_event
            )
